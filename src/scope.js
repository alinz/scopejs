(function () {
    ...
    function asyncMap(arr, func, done) {
        ...
        if (!length) task(function () { done(results); });
        else {
    ...
    function queueOrGet(name, func) {
        if (!modules[name]) {
            ...
            scope.get(name, function (o) {
                if (o) modules[name] = {o: o};
                func();
            });
        } else {
            if (modules[name].o !== undefined) func();
            else {
                if (!queues[name]) queues[name] = [];
                queues[name].push(func);
            }
    ...
    function scope() {
        ...
        if (obj.n) {
            ...
            if (target && target.d) throw "Err2: " + obj.n;
            modules[obj.n] = obj;
        }
